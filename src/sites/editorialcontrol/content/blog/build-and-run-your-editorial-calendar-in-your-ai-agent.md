...
![The Editorial Studio, issue cover №04. Both sites folded into one view ('AC' for audiocontrol.org, 'EC' for editorialcontrol.org), a stage filter across the content lifecycle (Ideas → Published), a voice-drift signal chip, and an inline shortform composer that assembles the `/editorial-shortform-draft` skill invocation. The Ideas section's 'intake new idea →' button opens the form that produced this very post's calendar row.](/images/blog/build-and-run-your-editorial-calendar-in-your-ai-agent/studio.png)

![The margin-note Review UI mid-iteration — a resolved comment from v5 shown struck-through in the sidebar's collapsed Resolved section, with the agent's "◆ addressed in v12" stamp and one-sentence reason beneath. Live comments sit inline against the rendered prose; the version strip along the top (v1 through v16) is the workflow's history.](/images/blog/build-and-run-your-editorial-calendar-in-your-ai-agent/review-ui.png)
